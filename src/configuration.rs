...
pub fn get_configuration() -> Result<Settings, config::ConfigError>  {
    ...
    // Determine the running environment or default to local
    tracing::info!("APP_ENVIRONMENT = {}", std::env::var("APP_ENVIRONMENT").unwrap_or_else(|_| "unknown".to_string()));
    let environment: Environment = std::env::var("APP_ENVIRONMENT")
        .unwrap_or_else(|_| "local".into())
        .try_into()
        .expect("Failed to parse APP_ENVIRONMENT");
    let env_file = format!("{}.yaml", environment.as_str());
    tracing::info!("env file name {:?} {env_file}", environment.as_str());
